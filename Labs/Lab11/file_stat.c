 ...
 * Modified by: L. Felipe Perrone, Modified by alex meijer
 * Date: 2011-4-21
...
int main(int argc, char *argv[]) {
  ...
  if (fstatvfs(fd, &fs_info) == 0) {
    ...
    printf(" file system block size: %lud\n", fs_info.f_bsize); 
    printf(" max. file name length: %lud\n", fs_info.f_namemax);
  } else {
  ...
  //edit: compatibility with symlinks added
  if (lstat(name, &file_info) == 0) {
    printf("\n== FILE INFO ============================\n");
    ...
    int stats = getpwuid_r(file_info.st_uid, &pwd, buf, 256, &result); 
    if(stats){
      perror("getpwid_r: ");
      exit(-1);
    }
    ...
    stats = getgrgid_r(file_info.st_gid, &grp, buf2, 256, &result2); 
    if(stats){
      perror("getgrgid_r: ");
      exit(-1);
    }
