...
/*     printf("exact match found. wiping node completely\n"); */
/* #endif */
/*     void* tobeallcd =  dlist_find_remove(free_list, targetspace); */
/*     dlist_add_front(allocated_list, tobeallcd ,thisspace); */
  
/*     return tobeallcd; */
/*   } else { //available space is bigger */
/*     void *spacetoalloc = targetspace; */

/*     //now change the pointer of availspace to point to the new fragment */
/* #ifdef DEBUG */
/*     printf("reducing availspace to fit the new data. Previous size of availspace: %d\n", dlist_get_datsize(free_list)); */
/* #endif */
/*     //some pointer arithmetic */
/*     targetspace = targetspace + size; */
...
/*     //now add the old avaliable space to the allocated list */
/*     dlist_add_front(allocated_list, spacetoalloc,size); */
...
double average_frag(){
  int sum_free_nodes = 0;
  
  dlist_iter_begin(free_list);
  
  sum_free_nodes = dlist_get_datsize(free_list);
  while(dlist_iter_has_next(free_list)){
    dlist_iter_next(free_list);
    sum_free_nodes = sum_free_nodes + dlist_get_datsize(free_list);
  }

  double avg = ((double) sum_free_nodes / (double) dlist_num_elems(free_list));
  return avg;
}
