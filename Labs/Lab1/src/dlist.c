 ...
 *
 * Author: L. Felipe Perrone (perrone@bucknell.edu), Alexander Meijer 
 */
...
dlist_add_front(struct dlist *l, void *ptr) {
  ...
  dnode_setdata(tempnode, ptr);
  
  if (l->front == NULL) { //empty list
    l->front = tempnode;
    l->back = tempnode;
...
dlist_remove_front(struct dlist *l) {
  ...
  if (l->front == NULL){
    ...
    struct dnode *tokill = l->front;
    l->front = l->front->next;
    l->front->prev = NULL;
   str =  dnode_destroy(tokill);
  }
        return str;
}
